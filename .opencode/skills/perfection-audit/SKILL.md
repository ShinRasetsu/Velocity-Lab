...
- **Fusion:** `IMU_ALPHA 0.15` adapt `0.08-0.35` (`index.html:1435`), `FUSION_BIAS_TAU_SEC 2.5` + `FUSION_NUDGE_TAU_SEC 0.8` dt-normalized gains, `FUSION_GPS_WEIGHT 0.12`, `IMU_BIAS_CLAMP 0.5` (`index.html:1453`), NIS innovation gate (3σ) + slew-limit ±3.5G·dt (filter always advances, never veto-freezes) + outlier-time (≥4 s, rate-independent) re-acquire from single-delta + window eviction; re-acquire seeds Doppler-or-0 on clamped dt (never gap-delta), `R` span capped 5 s; velocity = Doppler trusted else `lsVelocity` window-5 weighted 1/acc², anchors pruned >10 s; LS slope propagated to window end with IMU (no ramp lag); IMU older than 1 s never predicts/coasts (stale-sensor guard); resetRun tares mount only when stationary; 1D Kalman `Q=(Q0+1.5|a|)²dt` (`Q0 0.7 IMU / 8.0 GPS`), `R` Doppler/LS, no median buffer
