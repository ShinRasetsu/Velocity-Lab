...
- Shell: `stage` flex-fill + landscape 3-col + wide (`1100px`+landscape) 3-zone cockpit via `display:contents` (session+G left, gauge center `--s-gauge ×1.35`, score+timers right, `max-width:none`, `100dvh` no-clip), `tactical 2col` `metric-grid 2col` `hazard-strip` 3px `repeating-linear-gradient` (`index.html:819`), `version-tag` `VER` `index.html:1080`
