...
- Viewport: uniform `--ui = min(vw/390, vh/844)` via `updateScale` (JS-owned, no media overrides) + exact-fit to measured child boxes + padding (`cur*(vh-20)/contentH`, floor 0.65, grow only post-boot/orientation) + hysteresis (ignore <80px height jitter, kills URL-bar rescale loop) + bounded passes only (init + 900ms + fonts.ready shrink-only; mid-flight remeasure pulses), `html font-size calc(16px*var(--s-text))`, per-element dials `--s-text/--s-gauge/--s-card/--s-gap` (all default `var(--ui)`), `dashboard-shell` `100svh/100dvh` + `env(safe-area-inset-*)` + `max-width calc(430px*var(--ui))` centered
