...
- Viewport: uniform `--ui = min(vw/390, vh/844)` via `updateScale` (JS-owned, no media overrides) + shrink-to-fit content (`cur*vh/scrollH`, 10px slack, floor 0.65) + hysteresis (ignore <80px height jitter, kills URL-bar rescale loop) + bounded passes only (init + 900ms post-boot; extra passes re-measure mid-flight layout and pulse), `html font-size calc(16px*var(--s-text))`, per-element dials `--s-text/--s-gauge/--s-card/--s-gap` (all default `var(--ui)`), `dashboard-shell` `100svh/100dvh` + `env(safe-area-inset-*)` + `max-width calc(430px*var(--ui))` centered
