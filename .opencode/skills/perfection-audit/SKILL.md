...
- **Fusion:** `IMU_ALPHA 0.15` adapt `0.08-0.35` (`index.html:1435`), `FUSION_BIAS_TAU_SEC 2.5` + `FUSION_NUDGE_TAU_SEC 0.8` dt-normalized gains, `FUSION_GPS_WEIGHT 0.12`, `IMU_BIAS_CLAMP 0.5` (`index.html:1453`), NIS innovation gate (3σ, 4σ without fresh IMU) + slew-limit car-envelope `4+1.2G·slewDt` (filter always advances, never veto-freezes) + outlier-time (≥4 s, rate-independent) re-acquire from filtered vMeas (Doppler/LS, never raw single-delta) + window eviction; seed/re-acquire Doppler/LS-or-0 on clamped dt (never gap-delta; single-delta divides by rawDt), `R` span capped 5 s / `R_MAX 60`; velocity = Doppler trusted else `lsVelocity` 3 s/12 window weighted 1/acc², anchors pruned >3 s; LS slope propagated to window end with IMU (no ramp lag); IMU older than 1 s never predicts/coasts (stale-sensor guard); resetRun tares mount only when stationary; 1D Kalman `Q=(Q0+1.5|a|)²dt` (`Q0 0.7 IMU / 2.2 GPS`), `R` Doppler/LS, no median buffer
...
- **Accuracy:** `GPS_STALE_MS` (done 8000), altitude lift in `gpsConfidence` (done +0.15 <12m), `FUSION_GPS_WEIGHT` adapt >5Hz (done), LS-window + Kalman velocity (done, replaces median-5/single-delta), `crossFraction` linear vs quadratic (`index.html:1602`), creep gate (done 0.3 high-conf)
