...
6. Gate 5 System: `manifest.json:8`, `sw.js` register `2128`, `beforeinstallprompt` `1390`, `wakeLock` `1405`+`visibilitychange 1418`, `DeviceMotionEvent.requestPermission` `2089`+`VelocityLab_ios_motion_granted`, `STORAGE_KEY` `1096`+`save 2087`+`load 1300`+`purge 1328`, `GPS_STALE 8000` `1889`, `ERR: PERMISSION_DENIED` `1721`
...
9. Gate 8 Live Tools (must use if `opencode.json:6` enabled): Playwright `browser_navigate` file + deployed `title:6` + `browser_snapshot` `speed-display:999` `text-scale-btn:995` + `browser_evaluate` `--text-scale` toggle `1449` + `localStorage` + `speedBarTransition:none:647` + `sw:2148` + `browser_take_screenshot` 390x844/929x861 + offline `page.route` abort → `appShellStrategy:99` — enabled but not called → FAIL; Figma `get_design` FIGMA_FILE_KEY → tokens `107-113,540,169` drift → FAIL; chrome-devtools `performance_start` → `hud-boot 0.28s:553` <400ms + `CLS <0.05`; Cross-check `ui-fluidity-audit` generic Gate 1 with `liveIds` `958-1051` — ratio `<0.08` → FAIL
10. Gate 9 Recommendations (Advisory, mandatory every run): output ranked list `P0/P1/P2` + `impact` + `effort Low/Med` + `file:line` — cover optimization (`saveSession`, `sw.js:76`, `will-change`, font, `peak-pulse:419`, `g-dot:734`), accuracy (`GPS_STALE_MS:1145`, `GPS_ACC_TRUST`, `IMU_ALPHA:1141`, `crossFraction:1602`, creep gate), features/UI (timer ghost, text-scale init `133/1468`, iOS re-ask, manifest shortcuts, HIG 44pt `:44`, landscape). Skip already-implemented items.
