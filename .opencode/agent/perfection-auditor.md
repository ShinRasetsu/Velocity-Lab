...
5. Gate 4 Telemetry: `crossFraction/lerp` `index.html:1505`, `conf>=GPS_QUALITY_MIN` `1696`, `IMU_ALPHA 0.15` `1435`, `FUSION_*` `1453` (`BIAS_TAU 2.5` + `NUDGE_TAU 0.8` dt-normalized), NIS gate + slew-limit +-3.5G*dt (always advances) + outlier-time (>=4 s) single-delta re-acquire + window eviction, `lsVelocity` 3 s/12 window weighted 1/acc² + 1D Kalman (no median), `>0.5m/s` `1672`; run `node tests/telemetry.test.js` (fallback `python tests/tests.py`) — exit !=0 FAIL, no SKIP allowed
6. Gate 5 System: `manifest.json:8`, `sw.js` register `2128`, `beforeinstallprompt` `1390`, `wakeLock` `1405`+`visibilitychange 1418`, `DeviceMotionEvent.requestPermission` `2089`+`VelocityLab_ios_motion_granted`, `STORAGE_KEY` `1096`+`save 2087`+`load 1300`+`purge 1328`, `GPS_STALE 8000` + `COAST<20s usable` + `FIX/WEAK` tiers + weak-fix anchor-only + `lastUsableGpsTime` anti-pin bleed `1889`, `ERR: PERMISSION_DENIED` `1721`
7. Gate 6 Design: `--ui min(vw/390,vh/844)` + dials `--s-text/gauge/card/gap`, `safe-area-inset` `189`, `panel` `162`, `scanlines 0.12` `154`, `BAR_MAX 200` `zone-60 30%` `655`, `--gauge 292px dial`, `hazard-strip` `819`, `version-tag` `1080` — single-file no build; FAIL on `--scale`/`--text-scale`/vw-clamp resurgence
