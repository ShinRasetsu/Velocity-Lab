...
try {
    ...
    $checks = @{
        ...
        "Larger gauge" = $content -match '--gauge:\s*calc\(273px \* var\(--s-gauge\)\)' -and $content -match '--gauge:\s*calc\(292px \* var\(--s-gauge\)\)'
        "Uniform scale" = $content -match '--ui:\s*1;' -and $content -match "setProperty\('--ui'" -and $content -notmatch '--text-scale'
