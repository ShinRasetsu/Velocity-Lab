...
1. `node tests/fluidity.test.js` must PASS (Gate 1) + `node tests/telemetry.test.js` must PASS (Gate 4) + `node tests/layout.test.js` must PASS (Gate 6) — `python tests/tests.py` is equivalent fallback.
...
- Speed estimator: Doppler trusted else `lsVelocity` 3 s/12 window (1/acc²) + 1D Kalman (adaptive `Q`, accuracy-weighted `R`); NIS gate + slew-limit ±3.5G·dt (always advances), outlier-time (≥4 s) single-delta re-acquire, window eviction — no median buffer; IMU data older than 1 s never predicts, coasts, or tares.
